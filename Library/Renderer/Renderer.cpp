...
void Renderer::renderFrame()
{
	...
	auto [result, imageIndex] = m_logicalDevice.getDevice().acquireNextImageKHR(m_swapChain.getSwapchain(),
																				UINT64_MAX,
...
{ 
	...
	std::memcpy(data.data(), m_vertices.data(), static_cast<size_t>(bufferSize)); //vertices should fullfil trival object specyfication?
...
{ 
	...
	ubo.proj = m_camera.getProjectionMatrix(
		...
		(m_swapChain.getSwapchainExtend2D().width
		 / static_cast<float>(m_swapChain.getSwapchainExtend2D().height)),
		0.1F, 100.0F
	);
...
void Renderer::createTextureImage()
{
	...
	m_imageManager.createImage(texWidth, texHeight, vk::Format::eR8G8B8A8Srgb, vk::ImageTiling::eOptimal,
    ...
    m_imageManager.copyBufferToImage(
        stagingBuffer,
        m_textureImage,
...
{ 
	m_textureImageView = m_imageManager.createImageView(
